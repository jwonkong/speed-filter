...
public:
    ...
    void Sampling(Node* sample,Node*& parent, int accel);
    bool collision(Node* sample, Node*& parent);
    float Filter(float cmd_acceleration, float velocity);
    float findPath(float start_x, float start_y, float start_v);
    void AddLinearFunctionPoints(float m, float b, float xEnd, float gap_in);
...
void PathFinder::Sampling(Node* sample, Node*& parent, int accel) {
...
float PathFinder::Filter(float cmd_acceleration, float velocity) {
    float x_new_start = 0.4;
    ...
    if (velocity > velocity_limit || !new_start_check || cmd_acceleration <= 0){
        if (velocity > velocity_limit) cout << "velocity_limit" << endl;
        else if (!new_start_check) cout << "Collision warning" << endl;
        else cout << "cmd_acceleration < 0" << endl;
        return Target_A =  min(findPath(0, 0, velocity), cmd_acceleration);
    }

    else {// 가속시 충돌검사 해야함
        Target_A = findPath(x_new_start, y_new_start, v_new_start);
        cout << "??? :" << Target_A << endl;

        if (Target_A < 0) {
            cout << "Still safe" << endl;    
            if (cmd_acceleration > 0) Target_A = 0;
            else Target_A = min((Target_A + cmd_acceleration)/2, cmd_acceleration);            
            // Target_A =  findPath(0, 0, velocity); 겹침
            return Target_A;
        }
    }
...
void PathFinder::AddLinearFunctionPoints(float m, float b, float xEnd, float gap_in) {
    float gap = gap_in;
    if (gap > 0.1) gap = 0.1;

    for (float x = 0; x <= xEnd + gap; x += gap) {
        float y = m * x + b;
...
///Object
        ...
        // pathFinder.AddQuadraticFunctionPoints(front_obj_a, front_obj_v, front_obj_s, VehicleState_speed/5, 2*distance_object_to_line/VehicleState_speed);
        pathFinder.AddLinearFunctionPoints(front_obj_v, front_obj_s, VehicleState_speed/5 + 3, 2*distance_object_to_line/VehicleState_speed);
        // pathFinder.AddPointsToLShape(1.5, 50, 1.5, 2, 0.1 ,2*distance_object_to_line);
        // pathFinder.AddPointsToLShape(1.2, 10, 1, 10, 0.1 ,2*distance_object_to_line);
        // pathFinder.AddPointsToLShape(1.2, 10, 1, 10, 0.1 ,2*distance_object_to_line);
        // pathFinder.AddPointsToLShape(0.8, 40, 2, 3, 0.1 ,2*distance_object_to_line);
        // pathFinder.AddPointsToLShape(1.5, 50, 1.5, 2, 0.1 ,2*distance_object_to_line);
        // pathFinder.AddPointsSquare(1.5, 47, 1.5, 2, 2*distance_object_to_line/VehicleState_speed ,2*distance_object_to_line);
        // pathFinder.AddPointsSquare(1.2, 10, 1, 10, 2*distance_object_to_line/VehicleState_speed ,2*distance_object_to_line);

        float Target_A = pathFinder.Filter(LongitudinalCommand, VehicleState_speed);
